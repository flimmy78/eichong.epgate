...
public class EpChargeService {
			...
			int orgNo,int fromSource,String actionIdentity,String token,byte [] cmdTimes)
	{
        ...
        if (orgNo == UserConstants.ORG_I_CHARGE) {
            ...
            map.put("cpyId", userRealInfo.getCpyId());
            map.put("levelId", userRealInfo.getLevelId());
        } else {
            map.put("cpyId", u.getCpyId());
            map.put("levelId", u.getLevel());
        }
	...
	public static void insertChargeWithConsumeRecord(int chargeUserId,
		...
		record.setThirdType(thirdType);
		record.setAccountId(getAccountId(chargeUserId));
		// 新增开始充电记录
		DB.chargingrecordDao.insertFullChargeRecord(record);
			...
			boolean exceptionData,BigDecimal discountAmt,int pkCouPon,int thirdType,BigDecimal servicePrice)
	{
		...
		if (!exceptionData) {
			...
			if (payMode == EpConstants.P_M_FIRST || consumeRecord.getUserOrgin() == UserConstants.ORG_CHAT) {
				order.setChorChargingstatus(ChargeOrderConstants.ORDER_STATUS_SUCCESS + "");
			} else {
				order.setChorChargingstatus(ChargeOrderConstants.ORDER_STATUS_DONE + "");
			}
    ...
    public static int getAccountId(int userId) {
        UserRealInfo userRealInfo = UserService.findUserRealInfo(userId);
        if (userRealInfo == null)
            return 0;
        return userRealInfo.getAccountId();
    }
	...
	private static void updateOrder(TblConsumeRecord consumeRecord) {
        ...
        UserCache userInfo = UserService.getUserIdByOrgNo(consumeRecord.getUserOrigin());
	...
	public static TblCoupon queryCoupon(int cpUserid,int cpLimitation,int consemeAmt,String epCode)
	{
		return queryCoupon(cpUserid,cpLimitation,0,consemeAmt,epCode);
	}
	public static TblCoupon queryCoupon(int cpUserid,int cpLimitation,int actActivityrule,int consemeAmt,String epCode)
	{
		Map<String,Object> map =  new HashMap<String,Object> ();
			...
			int totalChargeTime,int totalChargeMeterNum,boolean isPauseStat)
	{
		...
		if(realCouponAmt != 0)//如果有优惠券抵扣
		{
			...
			{
				orgNo = companyList.get(0).getPkCompanyId();
			}

		    if (consumeRecord.getType() == 0) {
		    	RateService.addCouponToDB(new BigDecimal(realCouponAmt),userId,orgNo,String.valueOf(discountType),epCode,chargeSerialNo,accountId);
		    } else {
	 ...
	 * @return
	 */
	public static TblCoupon findCoupon(int usrId,int currentType,int acStatus,int dcStatus,int totalAmt,String epCode)
	{
		try
		{
	...
	public static void updateChargeRecordStatus(String chargeSerialNo,int status)
	{
		try
		{
			TblChargingOrder order = new TblChargingOrder();
			order.setChorTransactionnumber(chargeSerialNo);
			order.setChorChargingstatus(ChargeOrderConstants.ORDER_STATUS_DELETE + "");
			DB.chargeOrderDao.updateStatus(order);
			...
			updateConsumeRecord(chargeSerialNo);
			...
			UserCache chargeUser)
	{
		...
		try
		{
			...
			tblCoupon = findCoupon(chargeUser.getId(),currentType,AcStatus,DcStatus,iTotalAmt,consumeRecord.getEpCode());
	...
	private static String getExtraData_CCZC(String epCode,int epGunNo,String usrLog,String extra,int chargeFlag,int successFlag,int ret,String transactionNumber)
	{
		...
		if (epCache == null || epGunCache == null) {
			...
			if (transactionNumber != null) {
				List<TblChargingOrder> chargeOrderList = DB.chargeOrderDao.selectChargeData(transactionNumber);
	...
	public static String getExtraData_TCEC(String epCode,int epGunNo,String extra,int chargeFlag,int successFlag,int retCode )
	{
		String extraData="";
		ElectricPileCache epCache = EpService.getEpByCode(epCode);
		EpGunCache epGunCache= EpGunService.getEpGunCache(epCode, epGunNo);
		...
		String sGunNo = "0"+epGunNo;
		String endInfo="0";//0:无1: 此设备不存在2: 此设备离线3-99: 自定义
		if(retCode== ErrorCodeConstants.EPE_GUN_NOT_LINKED)
			endInfo="3";
		else if(retCode==ErrorCodeConstants.EP_UNCONNECTED)
			endInfo="2";
		...
		String sSuccessFlag = ""+successFlag;
		String[] array = {extra,sStatus,epCode+sGunNo,sSuccessFlag,endInfo};
 		extraData=StringUtils.join(array, "|");

		logger.info("TCEC epCode:{},epGunNo:{}, extraData:{}", new Object[] {
				epCode, epGunNo, extraData });
		return extraData;
	}
	...
	public static String getExtraData_resp_TCEC(String epCode,int epGunNo,String extra,int chargeFlag)
	{
		...
		String time=DateUtil.StringYourDate(new Date()); //时间
		String[] array = {extra,sStatus,epCode+sGunNo,time,""};
		extraData=StringUtils.join(array, "|");
	...
	public static int calcChargeAmt(RateInfo rateInfo, ConsumeRecord consumeRecord)
	{
		try {
			...
			value = NumUtil.intToBigDecimal3(consumeRecord.getfDl()).multiply(rateInfo.getF_Rate());
			consumeRecord.setfAmt(NumUtil.BigDecimal4ToInt(value));
			value = NumUtil.intToBigDecimal3(consumeRecord.getpDl()).multiply(rateInfo.getP_Rate());
			consumeRecord.setpAmt(NumUtil.BigDecimal4ToInt(value));
			value = NumUtil.intToBigDecimal3(consumeRecord.getgDl()).multiply(rateInfo.getG_Rate());
			consumeRecord.setgAmt(NumUtil.BigDecimal4ToInt(value));
			consumeRecord.setTotalChargeAmt(consumeRecord.getjAmt()+consumeRecord.getfAmt()+consumeRecord.getpAmt()+consumeRecord.getgAmt());
			if (rateInfo.getServiceRate() != null) {
				value = NumUtil.intToBigDecimal3(consumeRecord.getTotalDl()).multiply(rateInfo.getServiceRate());
				consumeRecord.setServiceAmt(NumUtil.BigDecimal4ToInt(value));
			}
			if (rateInfo.getJ_RateMoney() != null) {
				value = NumUtil.intToBigDecimal3(consumeRecord.getjDl()).multiply(rateInfo.getJ_RateMoney());
				consumeRecord.setjMoney(NumUtil.BigDecimal4ToInt(value));
			}
			if (rateInfo.getF_RateMoney() != null) {
				value = NumUtil.intToBigDecimal3(consumeRecord.getfDl()).multiply(rateInfo.getF_RateMoney());
				consumeRecord.setfMoney(NumUtil.BigDecimal4ToInt(value));
			}
			if (rateInfo.getP_RateMoney() != null) {
				value = NumUtil.intToBigDecimal3(consumeRecord.getpDl()).multiply(rateInfo.getP_RateMoney());
				consumeRecord.setpMoney(NumUtil.BigDecimal4ToInt(value));
			}
			if (rateInfo.getG_RateMoney() != null) {
