 ...
 */
public class TCECEASTService {

    private static final Logger logger = LoggerFactory.getLogger(TCECEASTService.class);
    ...
    private static void send2TCECEAST(String url, Map<String, Object> params) {
        TokenModel tokenModel = getTokenModel(UserConstants.ORG_TCEC_DongFeng);
    ...
    public static void startPushTimeout(long initDelay) {
        ...
        CheckTCECEASTPushTask checkTask = new CheckTCECEASTPushTask();
        logger.info("数据获取:  Period:{}",CooperateFactory.getCoPush(UserConstants.ORG_TCEC_DongFeng).getPeriod());
        TaskPoolFactory.scheduleAtFixedRate("TCEC_East_PUSH_TIMEOUT_TASK", checkTask, initDelay,
                CooperateFactory.getCoPush(UserConstants.ORG_TCEC_DongFeng).getPeriod(), TimeUnit.SECONDS);
    }
            ...
            while (iter.hasNext()) {
                ...
                if (val.length == 3) {
                    if (KeyConsts.STATUS_CHANGE_URL.equals(val[2])) {
                        send2TCECEAST(CooperateFactory.getCoPush(UserConstants.ORG_TCEC_DongFeng).getStatusChangeUrl(), pointMap);
                    } else if (KeyConsts.ORDER_URL.equals(val[2])) {
                        send2TCECEAST(CooperateFactory.getCoPush(UserConstants.ORG_TCEC_DongFeng).getOrderUrl(), pointMap);
                    } else if (KeyConsts.STOP_CHARGE_RESP_URL.equals(val[2])) {
                        send2TCECEAST(CooperateFactory.getCoPush(UserConstants.ORG_TCEC_DongFeng).getStopchargeRespUrl(), pointMap);
                    } else if (KeyConsts.CHARGE_RESP_URL.equals(val[2])) {
                        send2TCECEAST(CooperateFactory.getCoPush(UserConstants.ORG_TCEC_DongFeng).getChargeRespUrl(), pointMap);
                    } else if (KeyConsts.REAL_DATA_URL.equals(val[2])) {
                        send2TCECEAST(CooperateFactory.getCoPush(UserConstants.ORG_TCEC_DongFeng).getRealDataUrl(), pointMap);
                    }
                }
                removeRealData(key);
            }
    ...
    private static TokenModel getTokenModel(int org) {
        ...
        tokenModel.setOrg(UserConstants.ORG_TCEC_DongFeng);
        tokenModel.setStaticToken("");
        tokenModel.setTokenAvailableTime(new Date());
        tokenModel.setOperatorId(TCECEASTPush.OPERATOR_ID);
        tokenModel.setOperatorSecret(TCECEASTPush.OPERATOR_SECRET);
        tokenModel.setDataSecret(TCECEASTPush.DATA_SECRET);
        tokenModel.setDataSecretIv(TCECEASTPush.DATA_SECRET_IV);
        tokenModel.setSigSecret(TCECEASTPush.SIG_SECRET);
        return tokenModel;
    }
