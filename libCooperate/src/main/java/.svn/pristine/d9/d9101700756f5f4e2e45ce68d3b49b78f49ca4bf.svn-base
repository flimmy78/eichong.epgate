...
public class TCECChuanHuaPush extends Push {
    private static final Logger logger = LoggerFactory.getLogger(LogUtil.getLogName(TCECChuanHuaPush.class.getName()));
    ...
    public boolean init(String filename) {
        ...
        orgNo = Integer.valueOf(conf.getEChongProperties(KeyConsts.ORG_NO, "9014"));
    ...
    public void onChargeEpResp(String token, int orgNo, String userIdentity, String epCode, int epGunNo, String extra, int ret, int errorCode) {
        ...
        try {
            ...
            TCECChuanHuaService.addRealData(resMap.get("ConnectorID") + Symbol.SPLIT + resMap.get("StartTime")
    ...
    public void onEpStatusChange(String token, int orgNo, String userIdentity, String epCode, int epGunNo
        ...
        try {
            ...
            TCECChuanHuaService.addRealData(connectorID + Symbol.SPLIT + DateUtil.getCurrentSeconds()
                    + Symbol.SPLIT + KeyConsts.STATUS_CHANGE_URL, ensData);

            logger.debug("onepstatuschange is put :{}", JSON.toJSONString(resMap.get("ConnectorID") + Symbol.SPLIT + DateUtil.getCurrentSeconds()
                    + Symbol.SPLIT + KeyConsts.STATUS_CHANGE_URL + ensData));
        } catch (Exception e) {
            logger.error(LogUtil.addExtLog("exception"), e.getMessage());
        }
    ...
    public void onRealData(String token, int orgNo, String userIdentity, String epCode, int epGunNo, int inter_type, float servicePrice, Map<String, Object> realData, String extra) {
        try {
            ...
            if (resMap.size() > 0) {
                TCECChuanHuaService.addRealData(resMap.get("ConnectorID") + Symbol.SPLIT + DateUtil.getCurrentSeconds()
    ...
    public void onChargeOrder(String token, int orgNo, String userIdentity, String epCode, int epGunNo,
        ...
        try {
            ...
            if (stopCharge.size() > 0) {
                TCECChuanHuaService.addRealData(stopCharge.get("ConnectorID") + Symbol.SPLIT + DateUtil.getCurrentSeconds()
            ...
            TCECChuanHuaService.addRealData(order.get("ConnectorID") + Symbol.SPLIT + DateUtil.getCurrentSeconds()
