 ...
 */
public class TCECNanRuiPush extends Push {
    private static final Logger logger = LoggerFactory.getLogger(LogUtil.getLogName(TCECNanRuiPush.class.getName()));
    ...
    public void onEpStatusChange(String token, int orgNo, String userIdentity, String epCode, int epGunNo, int inter_type, Map<String, Object> realData, String extra) {
        try {
            ...
            logger.info("StatusChange is put  key:{}|realData{}", JSON.toJSONString(ensData), connectorID + Symbol.SPLIT + UserConstants.ORG_TCEC_NANRUI
                    + Symbol.SPLIT + KeyConsts.STATUS_CHANGE_URL);
            TCECENanRuiService.addRealData(connectorID + Symbol.SPLIT + UserConstants.ORG_TCEC_NANRUI
            ...
            if (resMap.get("Status").equals(255)) {
                ...
                TCECENanRuiService.addRealData(resMap.get("ConnectorID") + Symbol.SPLIT
    ...
    public void onRealData(String token, int orgNo, String userIdentity, String epCode, int epGunNo, int inter_type, float servicePrice, Map<String, Object> realData, String extra) {
        try {
            logger.info(LogUtil.addBaseExtLog("realData"), new Object[]{LogConstants.FUNC_ONREALDATA, epCode, epGunNo, orgNo, userIdentity, token, servicePrice, realData, extra, inter_type});
            ...
            Map<String, Object> ensData = new HashMap<>();
            ensData.put("ConnectorStatusInfo", resMap);
            TCECENanRuiService.addRealData(resMap.get("ConnectorID") + Symbol.SPLIT + UserConstants.ORG_TCEC_NANRUI
                    + Symbol.SPLIT + KeyConsts.REAL_DATA_URL, ensData);
            if (resMap.get("Status").equals(255)) {
                if (0 == statusConver(realData)) {
                    return;
                }
                ...
                Map<String, Object> encData = new HashMap<>();
                encData.put("AlarmInfos", list);
                logger.info("onRealData is put  key:{}|realData{}", resMap.get("ConnectorID") + Symbol.SPLIT + UserConstants.ORG_TCEC_NANRUI
                        + Symbol.SPLIT + KeyConsts.REAL_DATA_URL,encData);
                TCECENanRuiService.addRealData(resMap.get("ConnectorID") + Symbol.SPLIT + UserConstants.ORG_TCEC_NANRUI
                        + Symbol.SPLIT + KeyConsts.REAL_DATA_URL, encData);
                logger.info(LogConstants.FUNC_ONREALDATA + "AlarmInfo is put :{}", JSON.toJSONString(resMap.get("ConnectorID") + Symbol.SPLIT
                        + Symbol.SPLIT + "alarm_info_url" + encData));
            }
        } catch (Exception e) {
            logger.error(LogUtil.addExtLog("exception"), e.getMessage());
