...
public class TCECEQingXiangPush extends Push {
    private static final Logger logger = LoggerFactory.getLogger(LogUtil.getLogName(TCECEQingXiangPush.class.getName()));
    ...
    public boolean init(String filename) {
        ...
        orgNo = Integer.valueOf(conf.getEChongProperties(KeyConsts.ORG_NO, "9019"));
    ...
    public void onChargeEpResp(String token, int orgNo, String userIdentity, String epCode, int epGunNo, String extra, int ret, int errorCode) {
        ...
        TCECEQingXiangService.addRealData(resMap.get("ConnectorID") + Symbol.SPLIT + DateUtil.getCurrentSeconds()
                + Symbol.SPLIT + KeyConsts.CHARGE_RESP_URL, resMap);
        logger.info("onstartcharge is put ", new Object[]{LogConstants.FUNC_ONSTARTCHARGE, resMap});
    }
    ...
    public void onEpStatusChange(String token, int orgNo, String userIdentity, String epCode, int epGunNo
        ...
        try {
            ...
            TCECEQingXiangService.addRealData(connectorID + Symbol.SPLIT + DateUtil.getCurrentSeconds()
    ...
    public void onRealData(String token, int orgNo, String userIdentity, String epCode, int epGunNo, int inter_type, float servicePrice, Map<String, Object> realData, String extra) {
        try {
            ...
            logger.info("onrealdata resMap is put,resMap:{} ", resMap);
            TCECEQingXiangService.addRealData(resMap.get("ConnectorID") + Symbol.SPLIT + DateUtil.getCurrentSeconds()
                    + Symbol.SPLIT + KeyConsts.REAL_DATA_URL, resMap);
            logger.info("onrealdata is put ");
        } catch (Exception e) {
            logger.error(LogUtil.addExtLog("exception"), e.getStackTrace());
        }
    }
    ...
    public void onChargeOrder(String token, int orgNo, String userIdentity, String epCode, int epGunNo,
        ...
        try {
            ...
            if (stopCharge.size() > 0) {
                TCECEQingXiangService.addRealData(stopCharge.get("ConnectorID") + Symbol.SPLIT + DateUtil.getCurrentSeconds()
            ...
            TCECEQingXiangService.addRealData(order.get("ConnectorID") + Symbol.SPLIT + DateUtil.getCurrentSeconds()
                    + Symbol.SPLIT + KeyConsts.ORDER_URL, order);

            logger.info("onchargeorder is put ", new Object[]{LogConstants.FUNC_ONCHARGEORDER, stopCharge, order});
        } catch (Exception e) {
            logger.error(LogUtil.addExtLog("exception"), e.getStackTrace());
        }
