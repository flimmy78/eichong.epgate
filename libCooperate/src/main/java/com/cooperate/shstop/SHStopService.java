...
public class SHStopService {
    ...
    public static String sendRealData(Map<String ,Object> params) {
    	logger.debug(LogUtil.addExtLog("REAL_DATA_URL"),CooperateFactory.getCoPush(UserConstants.ORG_SHSTOP).getRealDataUrl());
    ...
    public static String sendOrderInfo(Map<String ,Object> params) {
    	logger.debug(LogUtil.addExtLog("ORDER_URL"),CooperateFactory.getCoPush(UserConstants.ORG_SHSTOP).getOrderUrl());
	...
	public static void checkSHSTOPPushTimeout()
	{
		try {
			...
			while (iter.hasNext()) {
				...
				if (val.length == 3) {
					if (KeyConsts.REAL_DATA_URL.equals(val[2])) {
